...
export const COMPANY_PROPERTIES = {
  ...
  lifecycleStage: "lifecyclestage",  // standard HubSpot property; filter to "onboarding" AND "pre-launch"
  avgL3mRevenue: "average_l3m_revenue", // TODO confirm — feeds GMV/travel rule + capacity weighting
  fullAddress: "full_address",       // confirmed name — powers the Travel tab's Location column
};

export const TICKET_PROPERTIES = {
  pipeline: "hs_pipeline",
  ticketStatus: "hs_pipeline_stage", // 15-stage pipeline — see STAGES list in the dashboard UI
  providerSegment: "provider_segment_pre_launch", // Enterprise / Gold / Silver / Experienced / Growth / Beginner sort
  ...
  currentEmr: "current_emr",         // matched against the Notion EMR tracker (see lib/notionClient.js)
  ...
  migrationNotes: "migration_notes", // confirmed name — shown in ticket detail + expandable rows
  ...
  migrationComplexityScore: "migration_complexity_score", // confirmed name
  migrationScopingDate: "migration_scoping_date",         // confirmed name — phase-duration + MoM charts
  serviceMenuMappingDate: "service_menu_mapping_date",    // confirmed name — phase-duration + MoM charts
  migrationCompleteDate: "migration_complete_date",       // confirmed name — completed log + avg days to complete
};
...
export async function fetchTickets({ limit = 100, after } = {}) {
  ...
  return res.results.map((t) => ({
    ...
    migrationNotes: t.properties[TICKET_PROPERTIES.migrationNotes],
    nextMeetingDate: t.properties[TICKET_PROPERTIES.nextMeetingDate],
    complexityScore: t.properties[TICKET_PROPERTIES.migrationComplexityScore]
      ? Number(t.properties[TICKET_PROPERTIES.migrationComplexityScore])
      : null,
    scopingDate: t.properties[TICKET_PROPERTIES.migrationScopingDate] || null,
    serviceMenuMappingDate: t.properties[TICKET_PROPERTIES.serviceMenuMappingDate] || null,
    completeDate: t.properties[TICKET_PROPERTIES.migrationCompleteDate] || null,
...
export async function fetchCompanies({ limit = 100, after } = {}) {
  ...
  return res.results.map((c) => ({
    ...
    fullAddress: c.properties[COMPANY_PROPERTIES.fullAddress] || null,
  }));
}

// Lifecycle filter used by the Launch Calendar tab — includes BOTH stages per your rule.
export const CALENDAR_LIFECYCLE_STAGES = ["onboarding", "pre-launch"];
