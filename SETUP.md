...
If terminal commands aren't working for you, GitHub's web UI supports uploading files
directly: on the repo page, Add file → Upload files → drag in the contents of this
folder (not the folder itself — its contents, so `lib/`, `pages/`, etc. carry over) →
commit to `main`. Skip `.DS_Store` if it shows up.
...
3. Before first deploy, add the environment variables from `.env.example` (Project →
   ...
   deploying, except Clerk's two keys (see Section 3) which the build fails without.
...
2. Clerk gives you two env vars — add both to Vercel (Project → Settings → Environment
   ...
   committed to GitHub. **The build will fail at the prerender step until both of these
   are set** — `_app.js` wraps every page in `ClerkProvider`, so a missing publishable
   key throws during static generation, not just at runtime.
...
- `middleware.ts` protects every page and API route except `/sign-in` (this repo uses
  ...
  Router always uses `middleware.ts`). The matcher includes `/__clerk/:path*` for
  Clerk's own auto-proxy requests, after the `(api|trpc)` line.
...
- **No public sign-up** — Clerk dashboard → User & Authentication → Restrictions →
  ...
  account at all.
- **Domain restriction in Clerk itself (do this too, not instead of the app check
  below)** — Clerk dashboard → User & Authentication → Restrictions → add
  `joinmoxie.com` under "Allowed email domains." This blocks anyone outside the domain
  from even completing sign-up, before your code ever runs. Combine with "no public
  sign-up" above if you want invite-only *within* the domain rather than open to anyone
  at Moxie.
- **Server-side check** — `lib/authz.js` now lets in *anyone* signed in with a
  `@joinmoxie.com` Google account by default (via `ALLOWED_EMAIL_DOMAINS`, defaults to
  `joinmoxie.com` even if the env var is unset) — no need to add people one at a time.
  The `ALLOWED_EMAILS` bootstrap list still exists for one-off exceptions outside the
  domain (e.g. a contractor), and Section 6a's Firestore roster is still checked as a
  further fallback for those individual cases.
...
**Clerk CLI, if you want to use it locally:** `npm install -g clerk`, then
`clerk auth login` (opens your browser — this has to be you, it's your Clerk account),
then `clerk init --app app_3GK6GyjJLcz3nSeYv7vbXA5Mi1u` to link this project to your
existing Clerk app, then `clerk doctor` to verify. Note `clerk init` is built primarily
for App Router projects — this repo is Pages Router, already set up manually per 3.2
above, so if the CLI prompts about App Router conventions or creates an `app/` folder,
stop and don't let it duplicate what's already in `pages/`.
...
2. Create a Service Account → generate a JSON key → this becomes
   `GOOGLE_SERVICE_ACCOUNT_JSON_BASE64` in Vercel (base64-encode it, since Vercel env
   vars are single-line strings).
...
## 8. Notion — EMR Confidence & Scraper tracker

**Who:** You, ~10 min. This is what powers the "already migrated" / travel-required /
scraper-needed logic (matching HubSpot's Current EMR against the "Known EMR Systems &
Nuances For Migration" Notion database).

1. Go to notion.so/my-integrations → **+ New integration** → name it "Migrations
   Dashboard" → Internal integration → Submit.
2. Copy the **Internal Integration Token** it gives you (starts with `secret_...`) →
   this is `NOTION_API_KEY`.
3. Open the actual "Known EMR Systems & Nuances For Migration" database in your browser
   → click **`...`** in the top right → **Connections** → add the "Migrations Dashboard"
   integration. (Skipping this step is the #1 cause of a 404 from the API — the
   integration can only see pages/databases it's explicitly been shared with.)
4. Grab the database ID out of the URL bar: it's the 32-character string right after
   your workspace name and before the `?v=`, e.g.
   `notion.so/joinmoxie/1a2b3c4d5e6f...?v=...` → the `1a2b3c4d5e6f...` part is
   `NOTION_EMR_DATABASE_ID`.
5. Add both `NOTION_API_KEY` and `NOTION_EMR_DATABASE_ID` to Vercel env vars (all
   environments) and to your local `.env.local`.
6. `lib/notionClient.js` expects three columns on that database: **System** (title or
   text), **Confidence Level** (select, with a value literally named "Not Yet Migrated"
   for anything not confirmed), and **Has Scraper** (checkbox or select). If your real
   column names differ, adjust the property-name strings in that file.

Any EMR that comes back from HubSpot's Current EMR field but isn't found anywhere in
this table is treated as **not yet migrated** — same as if it were found with
"Not Yet Migrated" explicitly set. That's intentional, per your rule that an EMR the
team hasn't documented yet shouldn't be assumed safe.

---

## 9. Nightly auto-refresh (12:00 AM ET)

**Who:** You or an engineer, ~5 min, once Sections 4/5/7/8 are all live.

1. `vercel.json` at the project root already defines a Vercel Cron job:
   `{ "crons": [{ "path": "/api/cron/refresh", "schedule": "0 5 * * *" }] }`.
   Vercel Cron schedules run in UTC — `0 5 * * *` is 5:00 AM UTC, which is midnight
   Eastern during daylight saving (adjust to `0 4 * * *` for EST months if you want it
   pinned exactly, or leave it and accept an hour of drift twice a year).
2. Cron jobs only run once a project is deployed on Vercel (not in local dev) and,
   depending on your plan, may need to be on a paid Vercel plan to run more than once a
   day — check your plan's cron limits before relying on this nightly.
3. Set a `CRON_SECRET` env var in Vercel; `pages/api/cron/refresh.js` checks incoming
   requests for `Authorization: Bearer <CRON_SECRET>` and rejects anything else, so the
   refresh endpoint can't be triggered by someone who finds the URL. Vercel automatically
   sends this header on its own cron-triggered requests once the env var is set.
4. What it does: pulls tickets + companies from HubSpot, pulls the EMR table from
   Notion, and writes both into Firestore (`tickets` collection + `settings/emrTable`
   doc) so the dashboard reads from Firestore instead of hitting HubSpot/Notion live on
   every page load.
